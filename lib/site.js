 ...
 */
function getJohariWindows(p, mf, yf, f){
	var data = {};
	data.name = p;
	data.openWindow = getOpenWindow(mf, yf);
	data.hiddenWindow = getHiddenWindow(mf, yf);
	data.blindWindow = getBlindWindow(mf, yf);
	data.unknownWindow = getUnknownWindow(mf, yf, f);
	return data;
}
...
function showJohariWindow(johariWindows, fs, share){
	var result = $("#result");
	var openWindow;
	var openWindowElm = "";
	var hiddenWindow;
	...
	for(var i=0; i<johariWindows.length; i++){
		...
		// social share button
		if(share){
			result.append("<div class='row'><div class='col-md-4'><button type='button' name='btnFbShare_"+i+"' class='btn btn-fb btn-block'>結果をFacebookでシェア</button></div><div class='col-md-4'><button type='button' name='btnTwShare_"+i+"' class='btn btn-tw btn-block'>結果をTwitterでシェア</button></div></div><br/>");
			result.append("<hr/>");
		}
